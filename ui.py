...
class TldrUI:
    def __init__(self):
        ...
        if 'query_input' not in st.session_state:
            st.session_state.query_input = "Summarize the key findings and recommendations."
    ...
    def process_files(self, files: List[Union[UploadedFile, BinaryIO]]) -> List[dict]:
        ...
        file_info = []
        for file in files:
            file_path = os.path.join("tldr_uploads", file.name)
...
async def main():
    ...
    with col1:
        ...
        if st.button("Refine Query", disabled=tldr_ui.processing):                
            await tldr_ui.tldr.refine_user_query(query)
        ...
        if documents is not None:
            if st.button("Process Input"):
                with st.spinner("Processing files..."):
                    ...
                    await tldr_ui.tldr.summarize_resources()
...
if __name__ == "__main__":
    os.makedirs("tldr_uploads", exist_ok=True)
    asyncio.run(main())