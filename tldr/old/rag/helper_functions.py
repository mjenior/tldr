...
def get_context(query, chunks, k=5):
    ...
    q_vec = chunks.embedding_function.embed_documents([query])
    _,indices = chunks.index.search(np.array(q_vec),k=k)

    texts = [idx_to_text(i, chunks) for i in indices[0]]
    return texts
...
def merge_pdf_library(directory: str = None):
    ...
    pdf_files = [file for file in files if file.endswith('.pdf') and not file.startswith('merged_library.')]
    if not pdf_files:
    ...
    output_filename = f"merged_library.{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.pdf"
    try:
        merger.write(output_filename)
        print(f"\nAll PDFs have been concatenated into {output_filename}")
    except Exception as e:
        print(f"\nFailed to write the concatenated PDF: {e}")
    finally:
        merger.close()
