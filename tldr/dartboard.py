...
class DartboardRetriever:
    def __init__(self, 
        ...
        """
        if not hasattr(chunks, 'embedding_function') or \
           not hasattr(chunks, 'index') or \
           not hasattr(chunks, 'index_to_docstore_id') or \
           not hasattr(chunks, 'docstore'):
            raise ValueError("Provided 'chunks' object does not appear to be a compatible vector store (e.g., FAISS).")
    ...
    def _lognorm(self, dist: np.ndarray, sigma: float) -> np.ndarray:
        ...
        """
        if sigma < 1e-9:
            return -np.inf * np.ones_like(dist) # Ensure it returns an array of the same shape
    ...
    def _greedy_dartsearch(
        ...
        while len(selected_documents) < num_results:
            if len(selected_documents) >= len(documents): # Break if we've selected all available docs
                break
            ...
            if document_probabilities.ndim == 1: # If only one candidate doc initially (edge case)
                 updated_distances = np.maximum(max_distances, document_probabilities)
            else:
                 updated_distances = np.maximum(max_distances[:, np.newaxis], document_probabilities)


            combined_scores = (
                updated_distances * div_weight +
                query_probabilities * rel_weight # query_probabilities might need broadcasting if it's 1D
            )
            ...
            normalized_scores[selected_indices_for_masking] = -np.inf # Mask already selected

            if np.all(np.isinf(normalized_scores)): # All remaining options are masked or invalid
                break
            ...
            selected_indices_for_masking = np.append(selected_indices_for_masking, best_idx)
    ...
    def get_context(
        ...
        oversampling_factor = oversampling_factor if oversampling_factor is not None else self.default_oversampling_factor
        ...
        query_distances = (1 - np.dot(np.array(query_embedding), candidate_vectors.T)).flatten()
